 ...
 * @param {Dictionary?} query        [The failed stage-three write query, with physical column names]
 *
 ...
 *                                @of {String}
 *                      @property {Array?} keys           [Physical columns reported by an E_UNIQUE error]
 *                      @property {Array?} keyValues      [Attempted values for those columns; bulk entries are candidates, not confirmed conflicts]
 *
 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 */
module.exports = function forgeAdapterError(err, omen, adapterMethodName, modelIdentity, orm, query){
    ...
    var customizations = (function(){
      ...
      switch (err.footprint.identity) {
        ...
        case 'notUnique': return (function(){
          ...
          // Some adapters (including sails-postgresql) discard the native conflicting value.
          // Recover only the reported columns from the failed write, after lifecycle callbacks
          // and column-name transformation. Never log unrelated fields or assume which row of
          // a bulk insert failed. Database-generated values might not be present in the query.
          var records = [];
          if (query) {
            records = adapterMethodName === 'createEach' ? query.newRecords : [query.newRecord || query.valuesToSet];
          }
          var keyValues = _.map(records, function(record) {
            return _.pick(record, err.footprint.keys);
          });
          var message = 'Would violate uniqueness constraint-- a record already exists with conflicting value(s).';
          message += ' Model: ' + modelIdentity + '. Keys: ' + JSON.stringify(err.footprint.keys) + '.';
          if (_.some(keyValues, function(values) { return _.keys(values).length > 0; })) {
            message += adapterMethodName === 'createEach' ? ' Candidate key/value sets: ' : ' Attempted key/value(s): ';
            message += JSON.stringify(adapterMethodName === 'createEach' ? keyValues : keyValues[0]) + '.';
          } else {
            message += ' Conflicting values were not supplied by the adapter or query.';
          }

          // Build the customizations for our uniqueness error. Keep the original message prefix,
          // code and attribute names so existing conflict handlers continue to recognize it.
          return {
            message: message,
            code: 'E_UNIQUE',
            attrNames: namesOfOffendingAttrs,
            keys: err.footprint.keys,
            keyValues: keyValues,
            toJSON: function (){
              return {
                code: this.code,
