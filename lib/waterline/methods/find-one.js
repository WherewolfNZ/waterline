      ...
      (function _maybeRunBeforeLC(proceed){
        ...
        helpFind(WLModel, query, omen, function _afterFetchingRecords(err, populatedRecords) {
          ...
          (function _maybeRunAfterLC(proceed){
            ...
            // If no afterFindOne callback defined, just proceed.
            if (!_.has(WLModel._callbacks, 'afterFindOne')) {
              return proceed(undefined, thePopulatedRecord);
            }//-•

            // If nothing matched, there is no record to hand to the callback, so
            // don't run it.  Otherwise every miss would call `afterFindOne` with
            // `undefined`, and a callback that touches the record - which is what
            // an "after findOne" callback is for - would throw on it.  Note that
            // this covers `findOrCreate()` too: the lookup it does before creating
            // comes through here, and the record it goes on to create is not one
            // this callback ran against.
            if (!thePopulatedRecord) {
              return proceed(undefined, thePopulatedRecord);
            }//-•

            // Otherwise, run it.
            return WLModel._callbacks.afterFindOne(thePopulatedRecord, function(err) {
              if (err) {
                return proceed(err);
              }
