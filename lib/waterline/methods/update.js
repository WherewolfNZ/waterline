  ...
  return parley(
    ...
    function (done){
      ...
      (function(proceed) {
        ...
        // `beforeUpdate` has two supported signatures: the original
        // `(valuesToSet, proceed)`, and `(valuesToSet, options, proceed)`, where
        // `options` exposes the query's `criteria` and `meta`.
        //
        // Which one a given model uses cannot be settled by `Function#length`.
        // That stops counting at the first parameter with a default value and
        // ignores rest parameters, so `(values, options, proceed = noop)` reports
        // 2 and `(...args)` reports 0. Guessing wrong is silent rather than loud:
        // a callback that wants the dictionary is handed `proceed` in its place,
        // then continues the query through a parameter that is `undefined` or a
        // default no-op, so the update hangs instead of failing.
        //
        // So nothing is guessed. Both arguments are always passed, and the second
        // one serves either purpose: it is a function that continues the query,
        // carrying the options as properties. A two-parameter callback calls it to
        // continue; a three-parameter callback reads `.criteria`/`.meta` off it and
        // continues with the third argument instead.
        var onward = _.once(function _onwardFromBeforeUpdate(err) {
          if (err) { return proceed(err); }
          return proceed(undefined, query);
        });

        var optionsOrOnward = function _continueTheUpdate(err) { return onward(err); };
        optionsOrOnward.criteria = query.criteria;
        optionsOrOnward.meta = query.meta;

        // Non-enumerable, so that `Object.keys()` on the dictionary still lists
        // just the options, and so that serializing it (when logging, say) yields
        // those rather than the `undefined` a bare function would.
        Object.defineProperty(optionsOrOnward, 'toJSON', {
          value: function _optionsToJSON(){
            return { criteria: query.criteria, meta: query.meta };
          }
        });

        WLModel._callbacks.beforeUpdate(query.valuesToSet, optionsOrOnward, onward);
        ...
        adapter.update(WLModel.datastore, query, function _afterTalkingToAdapter(err, rawAdapterResult) {
          if (err) {
            err = forgeAdapterError(err, omen, 'update', modelIdentity, orm, query);
            return done(err);
          }//-•
